...
// backend/api-docs/openAPIRouter.ts
...
var complaint_routes_default = router3;
...
var response_routes_default = router5;
...
// backend/api-docs/openAPIRouter.ts
var openAPIRouter = import_express6.default.Router();
...
app.use("/api/complaints", complaint_routes_default);
app.use("/api/responses", response_routes_default);
app.use("/api/agencies", agency_routes_default);
app.use("/api/users", user_routes_default);
app.use("/api/docs", openAPIRouter);
if (env.NODE_ENV === "production") {
  app.use(import_express7.default.static(import_path.default.join(__dirname, "../frontend/build")));
  app.get("*", (req, res) => {
    res.sendFile(import_path.default.resolve(import_path.default.join(__dirname, "../frontend/dist/index.html")));
  });
}
...
async function connectToDatabase() {
  try {
    await import_mongoose5.default.connect(env.MONGODB_URI);
    logger2.info("Connected to MongoDB");
  } catch (error) {
    logger2.error("Failed to connect to MongoDB", error);
    process.exit(1);
  }
}
import_mongoose5.default.connection.on("disconnected", () => {
  logger2.warn("Disconnected from MongoDB");
});
import_mongoose5.default.connection.on("error", (err) => {
  logger2.error("MongoDB connection error", err);
});
process.on("SIGINT", async () => {
  await import_mongoose5.default.connection.close();
  logger2.info("MongoDB connection closed due to app termination");
  process.exit(0);
});
